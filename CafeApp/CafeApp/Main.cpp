...
///
/// We set up our application boilerplate here.
///
void Init() {
  ///
  ...
  ///
  window = ulCreateWindow(ulAppGetMainMonitor(app), 500, 500, false, kWindowFlags_Titled | kWindowFlags_Resizable);
...
void OnDOMReady(void* user_data, ULView caller, unsigned long long frame_id,
  ...
  ///
  JSContextRef ctx = ulViewLockJSContext(view); //lock JS context
